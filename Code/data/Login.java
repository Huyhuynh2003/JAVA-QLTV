...
public class Login extends JFrame {
	...
	public Login() {
		...
		btnThot.setBounds(517, 309, 137, 38);
				...
				if (textField.getText().equals("")) {
					...
					try {
						...
						if (rs.next() && rdbtnNewRadioButton.isSelected()) {
							...
							GDdocgia dg = new GDdocgia( textField.getText());
							dg.setVisible(true);
						} else {
		...
		btnNewButton.setBounds(517, 261, 137, 38);
		contentPane.add(btnNewButton);
		
		JCheckBox hidepassCheckbox = new JCheckBox("Ẩn/hiện mật khẩu");
		hidepassCheckbox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (hidepassCheckbox.isSelected()) {
		...
		hidepassCheckbox.setFont(new Font("Times New Roman", Font.PLAIN, 16));
		hidepassCheckbox.setBounds(471, 230, 217, 30);
		contentPane.add(hidepassCheckbox);
	}
}